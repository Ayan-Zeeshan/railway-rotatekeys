        ...
        for doc in docs:
            ...
            for k, encrypted_val in doc_data.items():
                if encrypted_val is None:
                    ...
                    try:
                        # Attempt decryption. If fails, treat as plaintext
                        decrypted_fields[k] = aes_decrypt(old_aes_key, encrypted_val)
                    except Exception:
                        print(f"⚠️ Field '{k}' in doc {doc.id} was not encrypted. Encrypting directly.", flush=True)
                        decrypted_fields[k] = encrypted_val  # treat as plaintext
